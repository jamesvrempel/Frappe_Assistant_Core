    ...
    try:
        ...
        if result.get("success"):
            ...
            frappe.logger("migration_hooks").warning(
                f"Tool cache refresh had issues: {error}"
            )
            
    except Exception as e:
        # Don't fail migration due to cache issues
        frappe.logger("migration_hooks").error(
            f"Failed to refresh tool cache after migration: {str(e)}"
        )
...
def before_migrate():
    ...
    try:
        ...
        # Don't fail migration due to cache issues
        frappe.logger("migration_hooks").warning(
            f"Failed to clear tool cache before migration: {str(e)}"
        )
...
def after_install():
    ...
    try:
        ...
        if result.get("success"):
            ...
            error = result.get("error", "Unknown error")
            frappe.logger("migration_hooks").warning(
                f"Tool discovery initialization had issues: {error}"
            )
            
    except Exception as e:
        frappe.logger("migration_hooks").error(
            f"Failed to initialize tool discovery: {str(e)}"
        )
...
def after_uninstall():
    ...
    try:
        ...
        frappe.logger("migration_hooks").warning(
            f"Failed to cleanup tool cache: {str(e)}"
        )
...
def on_app_install(app_name: str):
    ...
    try:
        ...
        if result.get("success"):
            frappe.logger("migration_hooks").info(
                f"Tool cache refreshed after {app_name} installation"
            )
        else:
            frappe.logger("migration_hooks").warning(
...
def on_app_uninstall(app_name: str):
    ...
    try:
        ...
        if result.get("success"):
            frappe.logger("migration_hooks").info(
                f"Tool cache refreshed after {app_name} uninstallation"
            )
        else:
            frappe.logger("migration_hooks").warning(

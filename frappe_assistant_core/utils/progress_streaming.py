...
class ProgressTracker:
    ...
    def update_progress(self, 
        ...
        update = ProgressUpdate(
            ...
            status=status,
            progress_percent=progress_percent if progress_percent is not None else (prev_update.progress_percent if prev_update else 0),
            current_step=current_step or (prev_update.current_step if prev_update else ""),
            total_steps=total_steps if total_steps is not None else (prev_update.total_steps if prev_update else 1),
            current_step_number=current_step_number if current_step_number is not None else (prev_update.current_step_number if prev_update else 1),
            message=message,
            error=error,
        ...
        with self._lock:
            for operation_id, tracker in self.active_trackers.items():
                ...
                if (latest_update and 
                    latest_update.status in [ProgressStatus.COMPLETED, ProgressStatus.FAILED, ProgressStatus.CANCELLED] and

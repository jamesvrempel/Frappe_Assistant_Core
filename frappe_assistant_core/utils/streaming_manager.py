...
Build unlimited analysis depth via progressive artifact updates."""
        ...
        try:
            ...
            if self.should_enforce_streaming():
                return self._format_enforced_streaming(
                    tool_name, result, arguments, custom_instructions, 
                    artifact_type, sections, lines, chars
                )
            else:
    ...
    def _get_artifact_suggestions(self, tool_name: str) -> tuple:
        ...
        if tool_name in ["analyze_frappe_data", "execute_python_code", "query_and_analyze"]:
            return "Data Analysis Report", ["Executive Summary", "Key Findings", "Detailed Analysis", "Recommendations"]
            ...
            return "Technical Documentation", ["Overview", "Search Results", "Technical Details", "Usage Notes"]
        elif tool_name == "create_visualization":
            return "Visualization Report", ["Chart Overview", "Data Insights", "Visual Analysis", "Recommendations"]
        else:
            return "Comprehensive Results", ["Summary", "Main Results", "Detailed Output", "Next Steps"]
    
    def _format_enforced_streaming(self, tool_name: str, result: str, arguments: Dict[str, Any], 
                                   custom_instructions: str, artifact_type: str, sections: list,
                                   lines: int, chars: int) -> str:
...
"""
    ...
    def _format_optional_streaming(self, tool_name: str, result: str, arguments: Dict[str, Any],
                                   custom_instructions: str, artifact_type: str, sections: list,
                                   lines: int, chars: int) -> str:

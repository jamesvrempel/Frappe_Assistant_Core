...
*Comprehensive analysis building in artifact - no response limits with this approach.*
    ...
    @staticmethod
    def build_streaming_response(tool_name: str, operation_type: str, raw_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tool_name in ["analyze_frappe_data", "execute_python_code", "query_and_analyze", "create_visualization"]:
            ...
            needs_streaming = (
                output_lines > 20 or  # More than 20 lines of output
                variable_count > 15 or  # More than 15 variables created
                data_rows > 100 or  # More than 100 data rows
                (output_lines > 10 and variable_count > 8)  # Medium complexity combination
            )
            
            if needs_streaming:
                # Format full content for artifact creation
                full_content = StreamingResponseBuilder._format_full_analysis_for_artifact(tool_name, raw_result)
...
"""
                ...
                return {
                    "content": [{
                        "type": "text", 
                        "text": artifact_trigger_response
                    }]
                }
            else:
...
"""
                ...
                return {
                    "content": [{
                        "type": "text",
                        "text": actual_content + streaming_tip
                    }]
                }
            ...
            if raw_result.get('variables'):
                ...
                for var_name, var_value in key_vars:
                    if isinstance(var_value, dict) and var_value.get('type') == 'dataframe':
                        ...
                        value_str = str(var_value)[:50] + "..." if len(str(var_value)) > 50 else str(var_value)
    ...
    def _format_full_analysis_for_artifact(tool_name: str, raw_result: Dict[str, Any]) -> str:
        ...
        if tool_name == "execute_python_code":
            ...
            content += "*Use this artifact to continue analysis, share insights, or build upon these results.*\n"
    ...
    def _format_actual_results(tool_name: str, raw_result: Dict[str, Any]) -> str:
        ...
        if tool_name == "execute_python_code":
            ...
            if raw_result.get('variables'):
                ...
                for var_name, var_value in raw_result['variables'].items():
                    if isinstance(var_value, dict) and var_value.get('type') == 'dataframe':
                        ...
                        value_str = str(var_value)[:100] + "..." if len(str(var_value)) > 100 else str(var_value)
            ...
            if raw_result.get('data') and len(raw_result['data']) > 0:
                text += f"**Sample Results (first 3 rows):**\n"
...
def format_tool_response(tool_name: str, raw_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    analysis_tools = ["analyze_frappe_data", "execute_python_code", "query_and_analyze", "create_visualization", "report_execute"]
    
    if tool_name in analysis_tools:
        return StreamingResponseBuilder.build_streaming_response(
            tool_name, 
            "data_analysis", 
            raw_result
        )

    ...
    def enable_plugin(self, plugin_name: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                self.logger.info(f"Plugin '{plugin_name}' enabled successfully with {len(plugin_tools)} tools")
                return True
        ...
        for tool_name in plugin_info.tools:
            try:
                ...
                if tool_class:
                    ...
                    if not deps_valid:
                        self.logger.warning(f"Tool {tool_instance.name} dependency validation failed: {deps_error}")
                        continue

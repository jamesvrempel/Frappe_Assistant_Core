...
class ResourceMonitor:
    ...
    def _get_default_limits(self) -> Dict[ResourceType, ResourceLimit]:
        ...
        try:
            ...
            # Fallback defaults
            return {
                ResourceType.CPU: ResourceLimit(ResourceType.CPU, 80.0, 60.0, "percentage", "CPU usage limit"),
                ResourceType.MEMORY: ResourceLimit(ResourceType.MEMORY, 1024.0, 768.0, "MB", "Memory usage limit"),
                ResourceType.EXECUTION_TIME: ResourceLimit(ResourceType.EXECUTION_TIME, 300.0, 240.0, "seconds", "Maximum execution time")
            }
            ...
            while not self._stop_monitoring.is_set():
                try:
                    ...
                    if elapsed > time_limit.limit_value:
                        limit_exceeded = f"Execution time limit exceeded: {elapsed:.1f}s > {time_limit.limit_value}s"
...
class EnhancedErrorHandler:
    ...
    def create_error_context(self, 
                           ...
                           tool_name: str,
                           exception: Exception,
                           additional_context: Dict[str, Any] = None) -> ErrorContext:
        ...
        frappe_context = {
            ...
            "site": frappe.local.site if hasattr(frappe, 'local') and hasattr(frappe.local, 'site') else 'Unknown',
            "request_id": frappe.local.request_id if hasattr(frappe, 'local') and hasattr(frappe.local, 'request_id') else None,
            "form_dict": dict(frappe.form_dict) if hasattr(frappe, 'form_dict') else {},
            "method": frappe.request.method if hasattr(frappe, 'request') and hasattr(frappe.request, 'method') else None,
            "url": frappe.request.url if hasattr(frappe, 'request') and hasattr(frappe.request, 'url') else None
        }
        ...
        return ErrorContext(
            ...
            resolution_suggestions=self.resolution_map.get(error_type, ["Review error details and try again"])
        )
...
def enhanced_execution_context(operation_id: str, tool_name: str, monitor_resources: bool = True):
    ...
    try:
        ...
        if resource_monitor:
            ...
            if (monitoring_summary.get("peak_cpu", 0) > 50 or 
                monitoring_summary.get("peak_memory", 0) > 500 or
                monitoring_summary.get("duration", 0) > 60):

...
from ..constants.viz_definitions import (
    CHART_TYPES, TEMPLATE_CATEGORIES, TIME_SPANS, 
    PERFORMANCE_LIMITS, ERROR_MESSAGES
)
...
def _auto_arrange_charts(charts: List[Dict]) -> List[Dict]:
    ...
    sorted_charts = sorted(charts, key=lambda x: (
        x.get("priority", "medium"),
        x.get("chart_type", "bar")
    ))

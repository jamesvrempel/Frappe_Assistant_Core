...
from ..constants.viz_definitions import CHART_TYPES, AGGREGATION_METHODS
...
class ChartSuggestionEngine:
    ...
    def suggest_charts_for_data(self, doctype: str, fields: List[str], 
                               data_sample: List[Dict], user_intent: str = "") -> List[Dict[str, Any]]:
    ...
    def _analyze_categorical_field(self, series) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "has_long_tail": len(value_counts) > 20 and value_counts.iloc[10:].sum() < value_counts.iloc[:10].sum() * 0.1
            }
    ...
    def _suggest_multi_field_charts(self, field_analysis: Dict, data_sample: List[Dict]) -> List[Dict[str, Any]]:
        """Suggest charts combining multiple fields"""
        suggestions = []
        
        try:
            numeric_fields = [name for name, info in field_analysis.items() if info.get("category") == "numeric"]
            categorical_fields = [name for name, info in field_analysis.items() if info.get("category") == "categorical"]
            temporal_fields = [name for name, info in field_analysis.items() if info.get("category") == "temporal"]
    ...
    def _rank_suggestions(self, suggestions: List[Dict], field_analysis: Dict, data_sample: List[Dict]) -> List[Dict]:
...
def get_chart_suggestions(doctype: str, fields: List[str], 

...
class DocumentTools:
    ...
    @staticmethod
    def list_documents(doctype: str, filters: Dict[str, Any] = None, fields: List[str] = None, limit: int = 20, debug: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                for field_name in ["creation", "modified", "owner", "modified_by"]:
                    if (meta.has_field(field_name) and 
                        field_name not in safe_fields and 
                        len(safe_fields) < 10):  # Limit fields to avoid issues
            ...
            if len(documents) == 0 and total_count > 0:
                response["warning"] = f"No documents returned but {total_count} exist in database. Possible permission or filter issues."
            ...
            return {"success": False, "error": str(e), "debug": {"doctype": doctype, "filters": filters, "fields": fields}}
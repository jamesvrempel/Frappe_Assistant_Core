...
class BasePlugin(ABC):
    ...
    def on_enable(self) -> None:
        ...
        info = self.get_info()
        self.logger.info(
            _("Plugin {0} enabled successfully").format(info["name"])
        )
    ...
    def on_disable(self) -> None:
        ...
        info = self.get_info()
        self.logger.info(
            _("Plugin {0} disabled successfully").format(info["name"])
        )
    ...
    def _check_permissions(self, required_permissions: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        if missing_perms:
            return False, _("Missing permissions for: {0}").format(
                ", ".join(missing_perms)
            )

...
def get_performance_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        db_stats = {
            "total_queries": frappe.db.sql("SELECT COUNT(*) as count FROM `tabQuery Log` WHERE creation > %s", 
                                         (frappe.utils.add_days(frappe.utils.now(), -1),))[0][0] if frappe.db.table_exists("Query Log") else 0
        }
        ...
        frappe.log_error(
            title=_("Performance Metrics Error"),
...
def export_configuration() -> Dict[str, Any]:
    ...
    try:
        ...
        config_export = {
            ...
            "available_tools": [{"name": t.name, "type": "core" if not hasattr(t, "plugin_name") else "plugin"} for t in tools],
            "discovered_plugins": [{"name": p.get("name"), "status": p.get("status", "unknown")} for p in plugins]
        }
        ...
        frappe.log_error(
            title=_("Configuration Export Error"),
...
def import_configuration(config_data: str) -> Dict[str, Any]:
    ...
    try:
        ...
        frappe.log_error(
            title=_("Configuration Import Error"),
    ...
    try:
        ...
        try:
            ...
            for plugin in plugins:
                if not plugin.get("can_enable", False):
                    diagnostics["diagnostics"]["warnings"].append(f"Plugin '{plugin.get('name')}' cannot be enabled")
        ...
        try:
            ...
            if settings.request_timeout < 30:
                diagnostics["diagnostics"]["recommendations"].append("Consider increasing request timeout for complex operations")
